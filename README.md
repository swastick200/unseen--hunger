...
The easiest deployment checklist is in [DEPLOY.md](C:\Users\SWASTICK\OneDrive\Desktop(1)\Unseen hunger\DEPLOY.md).
...
Do not commit the real admin token either. Set your own secret token in Vercel or your local environment.
If `git` is not installed on your PC, use GitHub Desktop to publish the folder instead.
...
- The Next.js app is the main deployment target. The `public/` site is a local static fallback.
